...
struct ncTEVFSOSSInfo {
    ...
    5: ncTOSSSiteInfo siteInfo,         // 存储服务的站点信息
    6: i32 type,                        // 界面调用固定传 0；0 表示当前站点可界面管理的对象存储，1 表示分站点同步的对象存储
}
...
struct ncTEVFSOSSBaseInfo {
    ...
    2: string ossName,                  // 对象存储的名字
    3: bool enabled,                    // 是否启用，true 启用；false 禁用
    4: ncTOSSSiteInfo siteInfo;         // 存储服务的站点信息
...
service ncTEVFS {
    ...
    */
    ...
    // 更新存储使用的反向代理信息
    void UpdateOSSProxyInfo ()
                             throws (1: EThriftException.ncTException exp),

    // 更新本节点服务内容中，存储使用的反向代理信息，由本服务UpdateOSSProxyInfo内对每个节点调用
    void UpdateLocalOSSProxyInfo ()
                                 throws (1: EThriftException.ncTException exp),
