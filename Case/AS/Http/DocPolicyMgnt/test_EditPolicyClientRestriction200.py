...
class Test_EditPolicy_ClientRestrictionCheck200(object):
    @allure.testcase("ID5687,用例名：编辑策略配置--非子域编辑客户端登录限制，编辑成功--返回200")
    ...
    @pytest.mark.parametrize("headers, jsondata", argvalues=JsonRead(
        "AS\\Http\\DocPolicyMgnt\\testdata\\test_EditPolicyClientRestriction200.json").dict_value_join())
    def test_EditPolicy_ClientRestrictionCheck200(self, jsondata, headers, teardown):
        '''
               用例描述：该用例用于测试编辑策略接口中“客户端登录限制配置项”编辑检查，详情可见禅道用例
               编辑策略接口，只关注了策略的编辑成功
               策略功能是否正常应用到子域以及子域功能的检验，在应用策略接口中覆盖
               '''
        # 代入策略id到编辑策略的path路径中
...
if __name__ == '__main__':
    pytest.main(['-q', '-v', 'test_EditPolicyClientRestriction200.py'])
