...
class Test_ApplyPolicyToChildMultiFactorAuth200(object):
    @allure.testcase("ID5712,用例名：应用策略配置--非子域应用双因子认证至子域成功--返回200")
    @pytest.mark.parametrize("jsondata, checkpoint", argvalues=JsonRead(
        "AS\\Http\\DocPolicyMgnt\\testdata\\test_ApplyPolicyToChildMultiFactorAuth200.json").dict_value_join())
    ...
    @pytest.mark.parametrize("jsondata, checkpoint", argvalues=JsonRead(
        "AS\\Http\\DocPolicyMgnt\\testdata\\test_ApplyPolicyToChildMultiFactorAuth200.json").dict_value_join())
    def test_ApplyPolicyToChildMultiFactorAuth200(self, jsondata, checkpoint, teardown):
        policyid = teardown[0]
        ...
        response = tc.client.Usrm_GetVcodeConfig()
        time.sleep(2)
        assert response.passwdErrCnt == checkpoint['passwderrcnt']


if __name__ == '__main__':
    pytest.main(['-q', '-v', 'test_ApplyPolicyToChildMultiFactorAuth200.py'])
