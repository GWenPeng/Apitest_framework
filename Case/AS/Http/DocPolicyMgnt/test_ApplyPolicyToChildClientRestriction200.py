...
class Test_ApplyPolicyToChildClientRestriction200(object):
    @allure.testcase("ID5703,用例名：应用策略配置--非子域应用客户端限制至子域成功--返回200")
    @pytest.mark.parametrize("index, jsondata, checkpoint", argvalues=JsonRead(
        "AS\\Http\\DocPolicyMgnt\\testdata\\test_ApplyPolicyToChildClientRestriction200.json").dict_value_join())
    ...
    @pytest.mark.parametrize("index, jsondata, checkpoint", argvalues=JsonRead(
        "AS\\Http\\DocPolicyMgnt\\testdata\\test_ApplyPolicyToChildClientRestriction200.json").dict_value_join())
    def test_ApplyPolicyToChildClientRestriction200(self, index, checkpoint, teardown):
        policyid = teardown[0]
        ...
        for name in childpolicy:
            if name["name"] == "client_restriction":
        ...
        assert response[index].osType == checkpoint['os_type']


if __name__ == '__main__':
    pytest.main(['-q', '-v', 'test_ApplyPolicyToChildClientRestriction200.py'])
