...
class Test_SetPolicyMultiAuth400(object):
    @allure.testcase("ID5483,用例名：设置策略内容--非子域双因子认证，配置参数错误--返回400")
    @pytest.mark.parametrize("url, jsondata, headers, checkpoint", argvalues=JsonRead(
        "AS\\Http\\DocPolicyMgnt\\testdata\\test_SetPolicyMultiAuth400.json").dict_value_join())
    def test_SetPolicyMultiAuth400(self, url, jsondata, headers, checkpoint):
        '''
...
if __name__ == '__main__':
    pytest.main(['-q', '-v', 'test_SetPolicyMultiAuth400.py'])
