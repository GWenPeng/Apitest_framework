...
class Test_EditPolicy_MultiFactorCheck200(object):
    @allure.testcase("ID5688,用例名：编辑策略配置--非子域编辑双因子，编辑成功--返回200")
    ...
    @pytest.mark.parametrize("headers, jsondata", argvalues=JsonRead(
        "AS\\Http\\DocPolicyMgnt\\testdata\\test_EditPolicyMultiFactor200.json").dict_value_join())
    def test_EditPolicy_MultiFactorCheck200(self, jsondata, headers, teardown):
        '''
               用例描述：该用例用于测试编辑策略接口中“双因子配置项”编辑检查，详情可见禅道用例
...
if __name__ == '__main__':
    pytest.main(['-q', '-v', 'test_EditPolicyMultiFactor200.py'])
