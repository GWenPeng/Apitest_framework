...
class Test_AddPolicy_SharingCheck201(object):
    @allure.testcase("ID5320,用例名：新增策略配置--内外链共享，配置成功--返回201")
    # 每条用例执行完成后执行，清除环境
    ...
    @pytest.mark.parametrize("jsondata,checkpoint", argvalues=JsonRead(
        "AS\\Http\\DocPolicyMgnt\\testdata\\test_AddPolicySharing201.json").dict_value_join())
    def test_AddPolicySharingCheck201(self,jsondata,checkpoint,teardown):
        # 新增策略
...
if __name__ == '__main__':
    pytest.main(['-q', '-v', 'test_AddPolicyShare201.py'])
