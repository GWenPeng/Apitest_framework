...
class Test_ApplyEditedPolicyToMultiChild200(object):
    ...
    def teardown(self, metadata_host):
        ...
        yield policyid, relation_id1, relation_id2, child_host1, child_host2, father_host
        # 解绑子域
    ...
    @pytest.mark.parametrize("jsondata, ", argvalues=JsonRead(
        "AS\\Http\\DocPolicyMgnt\\testdata\\test_ApplyEditedPolicyToMultiChild200.json").dict_value_join())
    def test_ApplyEditedPolicyToMultiChild200(self, jsondata, teardown):
        policyid = teardown[0]
        relation_id1 = teardown[1]
        relation_id2 = teardown[2]
        child_host1 = teardown[3]
        child_host2 = teardown[4]
        father_host = teardown[5]
        # 应用策略至子域
        ...
        status = CommonDocPolicyMgnt().GetAllPolicyInfo(host=child_host2)["data"]
        for name in childpolicy:
            if name["name"] == "password_strength_meter":
                assert name["locked"] == True
...
if __name__ == '__main__':
    pytest.main(['-q', '-v', 'test_ApplyEditedPolicyToMultiChild200.py'])
