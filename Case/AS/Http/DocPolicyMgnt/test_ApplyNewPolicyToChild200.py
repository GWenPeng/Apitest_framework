...
class Test_ApplyNewPolicyToChild200(object):
    @allure.testcase("ID5375,用例名：应用策略配置--非子域应用策略至子域成功--返回200--新部署策略，应用到单个子域")
    # 每条用例执行完成后执行，清除环境
    ...
    def test_ApplyNewPolicyToChild200(self, teardown):
        policyid = teardown[0]
...
if __name__ == '__main__':
    pytest.main(['-q', '-v', 'test_ApplyNewPolicyToChild200.py'])
