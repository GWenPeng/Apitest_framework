...
class Test_Domain_AddPolicy_ClientRestrictionCheck201(object):
    @allure.testcase("ID5316,用例名：新增策略配置--客户端登录限制，配置成功--返回201")
    # 每条用例执行完成后执行，清除环境
    ...
    @pytest.mark.parametrize("jsondata, checkpoint",
                             argvalues=JsonRead("AS\\Http\\DocPolicyMgnt\\testdata"
                                                "\\test_AddPolicyClientRestriction201.json").dict_value_join())
    def test_Domain_AddPolicy_ClientRestrictionCheck201(self, jsondata,checkpoint,teardown):
        '''
        用例描述：该用例用于测试新增策略接口body部分传参“客户端登录限制”参数检查，详情可见禅道用例
        新增策略接口，只关注了策略的新增成功
...
if __name__ == '__main__':
    pytest.main(['-q', '-v', 'test_AddPolicyClientRestriction201.py'])
