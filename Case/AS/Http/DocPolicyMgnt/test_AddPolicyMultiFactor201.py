...
class Test_AddPolicy_MultiFactorCheck201(object):
    @allure.testcase("ID5317,用例名：新增策略配置--双因子认证，配置成功--返回201")
    # 每条用例执行完成后执行，清除环境
    ...
    @pytest.mark.parametrize("jsondata,checkpoint", argvalues=JsonRead(
        "AS\\Http\\DocPolicyMgnt\\testdata\\test_AddPolicyMultiFactor201.json").dict_value_join())
    def test_AddPolicy_MultiFactorCheck201(self, jsondata,checkpoint,teardown):
...
if __name__ == '__main__':
    pytest.main(['-q', '-v', 'test_AddPolicyMultiFactor201.py'])
