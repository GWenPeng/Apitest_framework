...
class Test_EditPolicy_NameCheck200(object):
    @allure.testcase("ID5334,用例名：编辑策略配置--非子域编辑策略名称和配置项成功--返回200 ")
    ...
    @pytest.mark.parametrize("headers, jsondata", argvalues=JsonRead(
        "AS\\Http\\DocPolicyMgnt\\testdata\\test_EditPolicyName200.json").dict_value_join())
    def test_EditPolicy_NameCheck200(self, jsondata, headers, teardown):
        # 策略id到编辑策略的path路径中
...
if __name__ == '__main__':
    pytest.main(['-q', '-v', 'test_EditPolicyName200.py'])
