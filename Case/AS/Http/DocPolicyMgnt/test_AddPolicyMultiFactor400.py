...
class Test_Domain_AddPolicy_MultiFactorCheck400(object):
    @allure.testcase("ID5323,用例名：新增策略配置--客户端登录限制，配置参数错误--返回400")
    @pytest.mark.parametrize("url, headers, jsondata, checkpoint", argvalues=JsonRead(
        "AS\\Http\\DocPolicyMgnt\\testdata\\test_AddPolicyMultiFactor400.json").dict_value_join())
    def test_Domain_AddPolicy_MultiFactorCheck400(self, url, jsondata, headers, checkpoint):
        '''
...
if __name__ == '__main__':
    pytest.main(['-q', '-v', 'test_AddPolicyMultiFactor400.py'])
